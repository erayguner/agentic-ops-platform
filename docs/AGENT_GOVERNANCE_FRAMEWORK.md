...
**Status:** Stable. Upstream-owned, downstream-adopted.
...
It is **portable**: every control is stated abstractly. An adopting
project provides its own implementation mapping (see "How to use this
document", below). Where this document references a shape like
`governed_call(request) -> result` or a file like
`core/tool_governor.py`, treat it as a behavioural pattern, not as a
path to copy verbatim from any specific codebase.

## How to use this document

The framework is owned **upstream as a single canonical document**.
An adopting project copies it into its own `docs/` tree (typically
`docs/AGENT_GOVERNANCE_FRAMEWORK.md`), pins it to a specific framework
version, and produces a **companion mapping document** that names the
file, module, Terraform resource, service account, queue, or policy
that implements each control in the project's own codebase.

Minimum adoption artefacts:

1. **A version-pinned copy** of this document at a stable path in the
   adopting repository.
2. **A project-specific governance-mapping document** (the project's
   own Appendix A) that links each framework section to the file /
   module / resource that implements it. The recommended filename is
   `GOVERNANCE-MAPPING.md`.
3. **A `governance-version` declaration** in the adopting repository's
   README naming the framework version the project claims conformance
   to (e.g. "Conformance: AGENT_GOVERNANCE_FRAMEWORK v1.1").
4. **A current §19 compliance attestation** kept up to date on each
   framework upgrade and on each control change. Unchecked boxes are
   acknowledged gaps, not oversights, and feed the operating cadence
   (§17.2).

Framework upgrades are handled per §20 (Framework lifecycle).
Adopters who choose not to upgrade still need to attest against the
version they last pinned.
...
20. [Framework lifecycle](#20-framework-lifecycle)

### Appendices

- [A — Reference implementation shapes](#appendix-a--reference-implementation-shapes)
- [B — External framework cross-reference](#appendix-b--external-framework-cross-reference)
- [C — Glossary](#appendix-c--glossary)
- [D — Autonomy tier crosswalk](#appendix-d--autonomy-tier-crosswalk)
- [E — A2A protocol compliance](#appendix-e--a2a-protocol-compliance)
...
**A2A protocol specifics.** Where adopters wire the Linux Foundation
**Agent2Agent (A2A) protocol v1.0** directly (rather than through a
provider-specific multi-agent surface like Bedrock multi-agent
collaboration or ADK sub-agents), the following protocol-level
requirements apply:

- **Agent cards.** Each A2A-exposing agent serves a signed agent card
  at the well-known URI `/.well-known/a2a-agent-card.json`. The card
  declares supported skills, expected input / output schemas per
  skill, supported authentication schemes, public identity, version,
  and contact / on-call. Card signatures are produced by the agent's
  per-agent identity (Vertex Agent Identity / Bedrock AgentCore
  Identity); unsigned cards are unacceptable in L2+.
- **Server-side authorisation.** The A2A server validates the
  incoming `Authorization` header against the caller's identity
  **before** dispatching to the agent's skill handler. Validation is
  server-side, not skill-side; a skill handler that has to
  re-authenticate its caller is a finding.
- **OAuth scope mapping.** Caller scopes (e.g. `read:findings`,
  `write:actions`) map to specific skills. A caller scoped to read
  cannot invoke a write skill regardless of role. Scope grants are
  declared in the upstream agent's boundary contract.
- **Trace propagation.** OpenTelemetry headers (`traceparent`,
  `tracestate`) flow across the A2A boundary so upstream and
  downstream spans share a `trace_id`. Each downstream `AgentStep`
  additionally carries `parent_agent_card_version` and
  `parent_correlation_id` referencing the upstream caller.
- **Catalogue drift detection.** The downstream agent's skill
  catalogue is fetched and hashed at session start; unexpected
  additions log a finding (mirrors the §4.5 MCP requirement).
...
**Prefer managed MCP fleets.** Where the provider operates a managed
MCP fleet, use it over a bespoke MCP server:

- **GCP:** the Google MCP fleet exposes Google service surfaces
  (BigQuery, Cloud Logging, Monitoring, Asset Inventory, Spanner,
  Workflows, Security Command Center, etc.) at
  `<service>.googleapis.com/mcp`. Authentication is IAM via the
  agent's identity; quota, audit, and Cloud Logging attach
  automatically; **Model Armor floor settings** inspect inputs and
  outputs at the fleet boundary regardless of caller template.
- **AWS:** **Bedrock AgentCore Gateway** hosts MCP-style tools with
  IAM-bound identity, CloudWatch logging, per-tool quota, and a
  managed credential vault for downstream API keys.

**Custom MCP servers** (written in-house) are tolerated where the
managed fleet does not cover the surface area. They must:

- Run as a **managed serverless service** (Cloud Run, Lambda, App
  Engine, Container Apps) — not a personal workstation or unmanaged
  VM.
- Authenticate callers via IAM / WIF / mTLS for same-cloud, OIDC
  federation for cross-cloud. No shared bearer tokens.
- Emit OpenTelemetry spans with `mcp.tool_name`, `mcp.server_url`,
  `mcp.caller_identity`, and `mcp.session_id` attributes.
- Treat the tool catalogue as an **API**: catalogue changes are
  semver-versioned and land under §16 change management.

**Tool-catalogue signing.** Tool catalogues are signed by the MCP
server's identity in high-trust deployments. The agent verifies the
signature on session start; unsigned catalogues are rejected at L3+.
...
The framework is **upstream-owned, downstream-adopted**. This section
sets out how it evolves, how adopters track changes, and how breaking
changes are handled.
...
- **Major (`X.0`)** — backward-incompatible changes to required
  controls. Existing adopters must re-attest against the new version
  before upgrading. Breaking-change rationale lives in the changelog.
- **Minor (`1.X`)** — additive controls, new appendices, clarifications
  that do not invalidate prior attestations. Existing adopters may
  upgrade without re-attesting but should review the new controls.
- **Patch (`1.1.X`)** — wording, typos, examples. No control semantics
  change.

### 20.2 Ownership

The framework has a single named owner upstream (the **Framework
owner** role, distinct from agent owners in §17.1). The Framework
owner is responsible for:

- Reviewing pull requests against the canonical document.
- Maintaining the changelog.
- Cutting versioned releases.
- Coordinating with adopters on breaking changes.

Adopters do **not** edit their pinned copy in-place; instead, they
raise a PR upstream and re-vendor when the change lands. Local
deviations from the canonical text are themselves a finding.

### 20.3 Deprecation

Controls are deprecated with a **one-release deprecation window**.
Deprecation is announced in the changelog with the rationale, the
replacement control, and the deadline. A deprecated control remains
required during its window; it becomes optional at the next minor
and removed at the next major.

### 20.4 Adopter responsibilities on upgrade

When a new framework version is published:

1. **Read the changelog.** Identify changes that affect the project's
   pinned conformance.
2. **Re-attest against §19** at the new version. Unchecked boxes are
   findings and feed the operating cadence (§17.2).
3. **Update the project README's `governance-version`** declaration.
4. **Update the project's governance-mapping document** for any new
   controls.

### 20.5 Compatibility with external frameworks

The framework's §2.1 alignment table is reviewed annually against the
latest published versions of SAIF, NIST AI RMF, EU AI Act, ISO/IEC
42001, SOC 2, and NCSC secure AI guidance. Updates land as minor
versions.

---

## Appendix A — Reference implementation shapes

This appendix lists the **behavioural shape** each control takes, in
language- and codebase-agnostic terms. An adopting project produces
its own mapping document (`GOVERNANCE-MAPPING.md` by convention) that
links each shape to the actual file, module, or resource implementing
...
- **Agent card.** Signed JSON document published by an A2A-exposing
  agent at `/.well-known/a2a-agent-card.json` declaring its skills,
  schemas, supported auth schemes, and public identity.
- **A2A.** Linux Foundation Agent2Agent protocol v1.0. The canonical
  surface for agent-to-agent delegation across runtimes; see §3.3 and
  Appendix E.
- **MCP.** Model Context Protocol. The canonical surface for an agent
  to invoke tools; see §4.5. Managed MCP fleets (Google MCP fleet,
  Bedrock AgentCore Gateway) are preferred over bespoke MCP servers.
- **Autonomy tier.** A per-action-class numeric (0–4) authority
  level used by some adopters in place of a single role per agent.
  Crosswalk to §3.1 in Appendix D.

---

## Appendix D — Autonomy tier crosswalk

The framework's §3.1 role taxonomy uses four named roles (Observer,
Advisor, Operator, Autonomous operator). Some adopting projects
prefer a **five-position numeric tier (0–4)** per action class
...
- **Per-action-class tier, per-environment.** A single agent may hold
  different tier ceilings for different action classes, and a single
  action class may be authorised at different tiers in `dev` vs `prod`.
  The boundary contract carries the matrix; promotion between tiers is
  a change-management event (§16).
- **Approval-gate primacy.** Where the numeric tier and the §3.1 role
  appear to disagree, the numeric tier is the operational decision;
  the role is the boundary-contract label. An agent labelled
  "Operator" in its boundary contract may hold action classes at
  Tier 0 (observe) and Tier 3 (act-approved) simultaneously.

Adopters using the numeric scheme should publish their full
action-class × environment × tier matrix as part of the change-
management surface (§16.1).

---

## Appendix E — A2A protocol compliance

The Agent2Agent (A2A) protocol (Linux Foundation, v1.0) is the
canonical surface for agent-to-agent delegation across runtimes. This
appendix is the A2A-specific compliance checklist that complements
§3.3 (multi-agent boundaries) and §19 (minimum compliance checklist).

### E.1 Agent cards

- [ ] Each A2A-exposing agent serves a signed **agent card** at
      `/.well-known/a2a-agent-card.json`.
- [ ] The agent card declares: supported skills, input / output
      schemas per skill, supported authentication schemes, public
      identity, version, and contact / on-call.
- [ ] Card signatures are produced by the agent's per-agent identity
      (Vertex Agent Identity / Bedrock AgentCore Identity).
- [ ] Card changes are change-management events (§16.2).

### E.2 Caller identity and authorisation

- [ ] The A2A server validates the `Authorization` header server-side
      before dispatching to the agent's skill handler.
- [ ] OAuth scopes (e.g. `read:findings`, `write:actions`) map to
      tools or skills; a caller's scope set is enforced at dispatch.
- [ ] Server-to-server calls use mTLS or signed JWTs; bearer tokens
      from a vault are acceptable only for tightly-scoped pull-only
      operations.

### E.3 Provenance and tracing

- [ ] Each downstream `AgentStep` carries `parent_agent_card_version`
      and `parent_correlation_id` linking back to the upstream
      caller.
- [ ] Trace propagation uses OpenTelemetry headers (`traceparent`,
      `tracestate`) such that the upstream and downstream spans share
      a `trace_id`.

### E.4 Approval gates at the A2A boundary

- [ ] An action triggered by a delegated A2A call inherits the
      approval requirements of the **downstream** agent's boundary
      contract, not the upstream caller's.
- [ ] Approval bundles emitted at the A2A boundary carry **both** the
      upstream and downstream agent identities; the audit chain links
      the two.

### E.5 Catalogue replay and drift

- [ ] The downstream agent's skill catalogue is fetched and hashed at
      session start; unexpected additions log a finding (matches §4.5
      MCP posture).
- [ ] Skill catalogue changes between two sessions are diffed in the
      audit log.
...
- **New §20 Framework lifecycle.** Versioning, ownership, deprecation,
  adopter upgrade responsibilities.
- **Beefed up §3.3 multi-agent boundaries.** A2A protocol specifics
  (agent cards at `/.well-known/a2a-agent-card.json`, server-side
  authorisation, OAuth scope mapping, OTel trace propagation, skill-
  catalogue drift detection).
- **Beefed up §4.5 MCP server posture.** Managed MCP fleet preference
  (Google MCP fleet at `<service>.googleapis.com/mcp`, Bedrock
  AgentCore Gateway), custom-MCP-server requirements, tool-catalogue
  signing.
- **Rewritten Appendix A.** No longer references a specific upstream
  implementation; provides generic behavioural shapes that adopters
  map to their own codebase in a project-specific
  `GOVERNANCE-MAPPING.md`.
- **New Appendix D Autonomy tier crosswalk.** Maps the framework's
  four-class role taxonomy (§3.1) to adopters' five-position numeric
  tier (0–4) schemes.
- **New Appendix E A2A protocol compliance.** A2A-specific checklist
  that complements §3.3 and §19.
- **Glossary additions.** Agent card, A2A, MCP, autonomy tier.
- **New "How to use this document" section.** Explicit framing of the
  framework as upstream-owned, downstream-adopted, with named
  adoption artefacts.

### v1.0 — initial release

Initial publication. 19 sections, 3 appendices, 7-framework external
cross-reference, full minimum-compliance checklist.
