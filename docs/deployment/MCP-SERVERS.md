...
| **FinOps**       | Monitoring, Recommender, BigQuery                                            | `monitoring.viewer`, `recommender.viewer`, dataset-scoped `bigquery.dataViewer` + `bigquery.jobUser` on the billing export                                                                                                |
| **Decommission** | Logging, Monitoring, Asset Inventory, Recommender                            | `logging.viewer`, `monitoring.viewer`, `cloudasset.viewer`, `recommender.viewer` (+ `resourcemanager.projectViewer` for API-side lookups; Resource Manager MCP stays excluded per the table below)                       |
...
| Gemini Cloud Assist                                                       | still Preview; avoid broad/ambiguous surface                                                                                                                        |
| Developer Knowledge, Agent Registry MCP                                   | now GA, but neither serves an agent purpose here — documentation lookup and registry introspection are not investigation surfaces                                    |
| ~~BigQuery MCP~~ **adopted**                                              | reached GA; wired to FinOps as the read-only billing-export surface always named as the first planned addition. IAM was already in place                             |
| ~~Recommender MCP~~ **adopted**                                           | reached GA; wired to FinOps and Decommission. `recommender.viewer` had already been granted to both in anticipation                                                  |
| Pub/Sub MCP                                                               | GA, but still excluded — agent SAs hold only `pubsub.publisher` on `ops.audit`, so its read surface would be denied. Needs a viewer grant first, not just an allow-list entry |
