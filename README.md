...
3. **`docs/GOVERNANCE-MAPPING.md`** — the AOP-specific Appendix A: every framework control mapped to the file / module / resource that implements it here, plus the current §19 compliance attestation **and the OWASP Top 10 for Agentic (ASI01–ASI10) + Non-Human Identity Top 10 crosswalk** (per the OWASP *State of Agentic AI Security and Governance* v2.01), **plus a Zero Trust for AI Agents capability-tier crosswalk** (Anthropic, 2026). Defensive operations (Part V — triage, MITRE ATT&CK coverage, session kill-switch, multi-incident tabletop, emergency-change procedures) live in [`docs/DEFENSIVE-OPERATIONS.md`](./docs/DEFENSIVE-OPERATIONS.md).
