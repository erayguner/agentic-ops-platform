...
3. **`docs/GOVERNANCE-MAPPING.md`** — the AOP-specific Appendix A: every framework control mapped to the file / module / resource that implements it here, plus the current §19 compliance attestation **and the OWASP Top 10 for Agentic (ASI01–ASI10) + Non-Human Identity Top 10 crosswalk** (per the OWASP _State of Agentic AI Security and Governance_ v2.01), **plus a Zero Trust for AI Agents capability-tier crosswalk** (Anthropic, 2026). Defensive operations (Part V — triage, MITRE ATT&CK coverage, session kill-switch, multi-incident tabletop, emergency-change procedures) live in [`docs/DEFENSIVE-OPERATIONS.md`](./docs/DEFENSIVE-OPERATIONS.md).
4. **`docs/aibom.yaml`** — the curated AI Bill of Materials (CycloneDX 1.6): the formal inventory of AI + agentic components (models, agent runtime, MCP servers, action-class tool connectors). The build-time dependency SBOM (CycloneDX + SPDX, Sigstore-signed) is generated by `.github/workflows/sbom.yml`.
5. **`terraform/FRAMEWORK.md`** — the reusable Terraform deployment framework: per-agent modules, composition module, pre-flight script, examples for `dev` / `staging` / `prod` and downstream consumers, release-please integration, and rollback runbook.
6. **`terraform/`**, **`agents/`**, **`services/`** — the three scaffolds, each with its own `README.md`. Cross-component schemas are defined inline in `agents/aop_common/schemas.py`, `services/slack-notifier/schemas.py`, and `services/action-broker/schemas.py`.
